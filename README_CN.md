...
| Tingfeng Wang | ADAS / TTC Risk | `motion_prediction`, `tools/generate_adas_from_gt.py` | 在基础 TTC 风险判断上优化 `motion_prediction`，加入 EKF 目标状态平滑和 CTRV / CA 短时轨迹预测，并将 FCW 高风险收敛为 TTC / closing speed 触发 |
...
| Tingfeng Wang / HanyueMo | Arbitration & HMI | `arbitration_module`, `hmi_interface` | Tingfeng Wang 负责 Arbitration，融合 ADAS、DMS、IQA 状态并生成统一告警与风险事件；HanyueMo 负责 HMI，完成展示层提示与可视化呈现 |
| ChenyukeWang | ROS2 & Protocol Bridge | `autodrivelab_msgs`, `arbiter_can`, `demo_bringup`, `autodrivelab_visualization` | 定义 ROS2 消息合同、Demo 级 CAN 映射、一键联调 launch 入口和 RViz2 3D MarkerArray 展示层 |
...
nuScenes scene replay
  ...
  -> final_demo.mp4 + optional RViz2 MarkerArray view
```
...
│   ├── autodrivelab_visualization/ # optional RViz2 3D MarkerArray view
...
# Optional RViz2 3D visualization
