...
DMS 测试使用本地座舱视频集 `data/DMS_test_data/`，通过 [DMS test demo runner](README_DMS_test_demo.md) 生成逐帧 `dms_status.jsonl` 和带风险状态叠加的 `dms_visualization.mp4`。测试数据和可视化输出属于本地运行产物，不提交到 GitHub；主页表格保留可复现的检测结果摘要。
